...
import { contentItems, popularKeywords, genres as allGenres } from "@/data/mockContent";
...
export const SearchPage = ({ myList, onToggleList }: SearchPageProps) => {
  useDocumentMeta(
    "Search | streamXie",
    "Search movies, series, anime, drama, and variety content instantly.",
  );
  ...
  // Collect genres available in current base results
    ...
    <PageContainer className="pt-32 pb-16">
      <section className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-4 text-base text-muted-foreground">
          Find movies, series, anime, drama, and variety with instant catalog filtering.
        </p>
        <form
          onSubmit={onSubmit}
        ...
        {/* Recent + popular keywords (shown when no query) */}
        {!initial && (
          <div className="mt-8 space-y-5">
                ...
                {recent.length > 0 ? (
                  recent.map((term) => (
                    <button
                      key={term}
                      type="button"
                      onClick={() => { setQuery(term); setParams({ q: term }); }}
            ...
            <div>
              <p className="mb-3 text-sm text-muted-foreground">Popular keywords</p>
              <div className="flex flex-wrap justify-center gap-3">
      ...
      {/* Filter + sort row — shown when there are base results */}
      <AnimatePresence>
