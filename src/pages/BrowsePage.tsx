...
export const BrowsePage = ({ myList, onToggleList }: BrowsePageProps) => {
  ...
  const chips = ["All", "Movies", "Series", "Anime", "Drama", "Variety"];

  const filtered = useMemo(() => {
    let items = [...contentItems];
    if (activeChip !== "All") items = items.filter((item) => item.category === activeChip);
  ...
  const resetFilters = () => {
    setFilters({ genre: "All", year: "All", type: "All", country: "All", status: "All", sort: "Popularity" });
    setActiveChip("All");
  };
  ...
  return (
    <PageContainer className="pt-32 pb-16">
      <SectionHeader
        title="Browse"
        subtitle={`${filtered.length} title${filtered.length !== 1 ? "s" : ""} found`}
        action={
          hasActiveFilters ? (
            <button
              type="button"
